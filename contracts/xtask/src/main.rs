...
pub mod tasks {
    ...
    pub fn generate_schemas() {
        macro_rules! generate_schema {
            ($contract:expr, $api_ref: expr) => {
                (
                    $contract.to_string(),
                    generate_api! {
                        instantiate: $api_ref::msg::InstantiateMsg,
                        query: $api_ref::msg::QueryMsg,
                        execute: $api_ref::msg::ExecuteMsg,
                        migrate: $api_ref::msg::MigrateMsg,
                    },
                )
            };
        }
        ...
        let contracts = metadata
            .packages
            .into_iter()
            .filter(|member| member.name != "xtask" && member.manifest_path.contains("contracts"));
